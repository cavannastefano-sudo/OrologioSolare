...
function isDSTInTimeZone(timeZone, date = new Date()) {
    try {
        ...
        const janOffset = getOffset(new Date(date.getFullYear(), 0, 15), timeZone);
        const julOffset = getOffset(new Date(date.getFullYear(), 6, 15), timeZone);

        // Se l'offset attuale è maggiore dell'offset minimo invernale, siamo in ora legale
