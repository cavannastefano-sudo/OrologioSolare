...
function applyTimezonePreset() {
    ...
    const totalTzOffset = parseFloat(tz) + (isDstNowActive ? 1 : 0);
    const tzSign = parseFloat(tz) >= 0 ? "+" : "";
    const totalSign = totalTzOffset >= 0 ? "+" : "";

    let fusoText = `Fuso solare: UTC ${tzSign}${tz}`;
    if (isDstNowActive) {
        fusoText += ` (Ora legale: UTC ${totalSign}${totalTzOffset})`;
    }

    document.getElementById('location-text').innerHTML = `<div style="font-size: 1.15rem; margin-bottom: 4px;">${fusoText}</div>`;
